"""Funciones de construcción de vistas (componentes Dash) para cada página."""
...
def to_slug(nombre):
    ...
    so that from_slug can distinguish hyphens-as-spaces from real hyphens.
    """
    s = nombre.strip()
    s = s.replace('-', '%2D')
    s = s.replace(' ', '-')
...
def from_slug(slug):
    ...
    Reverses to_slug: hyphens become spaces, %2D becomes literal hyphen.
    """
...
# ============================================================
# Building blocks (slides, carrusel, secciones)
# ============================================================
...
def crear_seccion_vendedor(df, vendedor, resumen_cerv=None):
    ...
    """
    vendor_id = to_slug(vendedor).lower()
...
def crear_seccion_sucursal(df, sucursal):
...
def crear_seccion_supervisor(df, supervisor, sucursal):
    ...
    carrusel = _crear_seccion_agregada(
        ...
        anchor_id=f'supervisor-{to_slug(supervisor).lower()}',
    )
    ...
    return _crear_bloque(
        ...
        href=f'/supervisor/{to_slug(supervisor)}{suc_param}',
    )


def crear_category_toggle():
...
def crear_bloque_vendedor(df, vendedor):
    ...
    return html.Div([
        html.H5(
            dcc.Link(titulo_content, href=f'/vendedor/{to_slug(vendedor)}', className='nav-link-title'),
            className='vendor-name',
        ),
        crear_seccion_vendedor(df, vendedor, resumen_cerv=resumen),
...
def render_vendedor_page(df, vendedor):
    ...
    for cat in [c for c in CATEGORIAS if c != 'CERVEZAS']:
        nombre = NOMBRES_CATEGORIA.get(cat, cat)
        r = get_resumen_vendedor(df, vendedor, cat)
        secciones.append(_crear_slide_otros([(nombre, r)]))
    vista_scrollable = html.Div(secciones, className='vendor-scrollable')
...
def render_supervisor_page(df, supervisor, sucursal=None):
    ...
    seccion_sup = crear_seccion_supervisor(df, supervisor, sucursal)
...
def render_sucursal_page(df, sucursal_param):
    """Vista de sucursal: total + supervisores con resumen."""
    sucursal = find_sucursal(df, sucursal_param)
    if not sucursal:
        return html.Div(f'Sucursal "{sucursal_param}" no encontrada', className='empty-state')

    seccion_suc = crear_seccion_sucursal(df, sucursal)
    supervisores = get_supervisores(df, sucursal)

    secciones = []
    for sup in supervisores:
        secciones.append(crear_seccion_supervisor(df, sup, sucursal))
...
def render_mapa_page(df, vendedor, search=None):
