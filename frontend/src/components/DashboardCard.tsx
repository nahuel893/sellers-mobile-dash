...
export default function DashboardCard({ card }: DashboardCardProps) {
  return (
    <Link
      to={card.href}
      className="block bg-bg-1 border border-line rounded-2xl p-5 hover:border-ink-2 active:scale-[0.98] transition-all no-underline"
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h2 className="text-base font-extrabold text-ink-0 leading-tight">{card.title}</h2>
          <p className="text-sm text-ink-2 mt-1 leading-snug">{card.description}</p>
        </div>
        <div className="flex-shrink-0 text-ink-3 self-center">
